...
class Form {
  ...
  processAuthRequest(postData) {
    ...
    // const serverURL = process.env.PORT || 'http://localhost:5300';
    // const apiPath = '/subscribe';
    ...
    this.storeInLocalStorage(postData);
    this.success();
    // this.error();
  }

  storeInLocalStorage(postData) {
    this.subscribers.push(postData);
    let arrayToString = JSON.stringify(this.subscribers);
    window.localStorage.setItem('storage', arrayToString);

    let deserializedArray = JSON.parse(window.localStorage.getItem('storage'));

    // This all the local data!!
